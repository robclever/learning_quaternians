...
pub mod demonstrations {
    ...
    pub fn demonstrate_gimbal_lock() {
        ...
        println!(
            "   Euler angles: roll={:.3}, pitch={:.3}, yaw={:.3} radians",
            normal_euler.roll, normal_euler.pitch, normal_euler.yaw
        );
        println!("   Analysis: {}", analysis.description);
        println!(
            "   Safety factor: {:.3}",
            GimbalLockDetector::gimbal_lock_safety_factor(&normal_euler)
        );
        println!();
        ...
        println!(
            "   Safety factor: {:.3}",
            GimbalLockDetector::gimbal_lock_safety_factor(&gimbal_euler)
        );
        println!();

        // Case 3: Show equivalence at gimbal lock
        // At pitch = +90° the orientation depends only on (yaw - roll), so triples
        // that move roll and yaw by the *same* amount describe one attitude...
        println!("3. Gimbal Lock Equivalence (pitch = +90°):");
        let euler1 = EulerAngles::new(0.5, PI / 2.0, 0.3);
        let euler2 = EulerAngles::new(1.0, PI / 2.0, 0.8);

        println!(
            "   Euler angles 1: roll={:.3}, pitch={:.3}, yaw={:.3} (yaw - roll = {:.3})",
            euler1.roll,
            euler1.pitch,
            euler1.yaw,
            euler1.yaw - euler1.roll
        );
        println!(
            "   Euler angles 2: roll={:.3}, pitch={:.3}, yaw={:.3} (yaw - roll = {:.3})",
            euler2.roll,
            euler2.pitch,
            euler2.yaw,
            euler2.yaw - euler2.roll
        );
        println!(
            "   Quaternions represent same orientation: {}",
            GimbalLockDetector::demonstrate_equivalence(&euler1, &euler2)
        );

        // ...while moving only one of the two angles lands somewhere else.
        let euler3 = EulerAngles::new(0.5, PI / 2.0, 0.8);
        println!("   Changing yaw alone (roll={:.3}, pitch={:.3}, yaw={:.3}) gives the same orientation: {}",
                 euler3.roll, euler3.pitch, euler3.yaw,
                 GimbalLockDetector::demonstrate_equivalence(&euler1, &euler3));
        println!();
    }
    ...
    pub fn demonstrate_gimbal_lock_approach() {
        ...
        for &pitch_deg in &test_angles {
            ...
            println!(
                "Pitch = {:5.1}°: Safety = {:.6}, Gimbal Lock = {}",
...
mod tests {
    ...
    fn test_gimbal_lock_detection() {
        ...
        assert!(analysis.is_gimbal_lock);
        assert!(matches!(
            analysis.singularity_type,
            SingularityType::PitchUp
        ));
    ...
    fn test_gimbal_lock_equivalence() {
        ...
        let euler1 = EulerAngles::new(0.5, PI / 2.0, 0.3);
        // At pitch = +90° the attitude depends only on (yaw - roll), so advancing
        // roll and yaw by the same amount describes an identical orientation.
        let euler2 = EulerAngles::new(1.0, PI / 2.0, 0.8);

        assert!(GimbalLockDetector::demonstrate_equivalence(
            &euler1, &euler2
        ));
        ...
        // Moving only one of the two angles breaks the invariant, even though the
        // other angle is untouched.
        let yaw_only = EulerAngles::new(0.5, PI / 2.0, 0.8);
        let roll_only = EulerAngles::new(1.0, PI / 2.0, 0.3);
        assert!(!GimbalLockDetector::demonstrate_equivalence(
            &euler1, &yaw_only
        ));
        assert!(!GimbalLockDetector::demonstrate_equivalence(
        ...
        // Looking straight down is the mirror singularity: there the attitude
        // depends on (yaw + roll) instead of (yaw - roll).
        let pitch_down_a = EulerAngles::new(0.2, -PI / 2.0, 0.6);
        let pitch_down_b = EulerAngles::new(0.9, -PI / 2.0, -0.1);
        assert!(GimbalLockDetector::demonstrate_equivalence(
            &pitch_down_a,
            &pitch_down_b
        ));
